...
const UserDashboard = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      updateRecentActivity(resumesData);
      setLoading(false);
    }, (error) => {
  ...
  const calculateStats = (resumesData) => {
    ...
    const lastWeekScores = resumesData
      .filter(r => r.lastModified && new Date(r.lastModified) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000))
      .map(r => r.atsScore || 0)
      .filter(s => s > 0);
    
    const prevWeekScores = resumesData
      .filter(r => r.lastModified && new Date(r.lastModified) > new Date(Date.now() - 14 * 24 * 60 * 60 * 1000))
      .filter(r => new Date(r.lastModified) < new Date(Date.now() - 7 * 24 * 60 * 60 * 1000))
      .map(r => r.atsScore || 0)
      .filter(s => s > 0);
    ...
    const scoreTrend = prevWeekAvg > 0 
    ...
    setStats({
      ...
      bestScore,
      totalDownloads,
      lastUpdated: resumesData.length > 0 
        ? resumesData.sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt))[0]?.updatedAt
        : null,
      scoreTrend
    });
  };

  const updateRecentActivity = (resumesData) => {
    const activities = [];
    ...
    resumesData.forEach(resume => {
      ...
      if (resume.lastDownloaded) {
        activities.push({
          id: `${resume.id}-downloaded`,
          type: 'downloaded',
          resume,
          timestamp: new Date(resume.lastDownloaded)
        });
      }
    });
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      toast.success(`${selectedResumes.size} resumes deleted successfully`);
  ...
  const filteredResumes = useMemo(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(r =>
        ...
        r.data?.personal?.title?.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                className="p-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white/50 dark:bg-gray-800/50 hover:bg-gray-100 dark:hover:bg-gray-700"
              >
                {sortDirection === 'asc' ? '↑' : '↓'}
              </button>
              ...
              <div className="flex bg-gray-100 dark:bg-gray-800 rounded-lg p-1">
                <button
                  ...
                  className={`p-2 rounded-md transition-all ${viewMode === 'grid' ? 'bg-white dark:bg-gray-700 shadow-sm text-primary-600' : 'text-gray-500'}`}
                >
                  <FiGrid className="w-4 h-4" />
                </button>
                <button
                  onClick={() => setViewMode('list')}
                  className={`p-2 rounded-md transition-all ${viewMode === 'list' ? 'bg-white dark:bg-gray-700 shadow-sm text-primary-600' : 'text-gray-500'}`}
                >
          ...
          {selectedResumes.size > 0 && (
            ...
            >
              <span className="text-sm">
                <span className="font-medium">{selectedResumes.size}</span> resumes selected
              </span>
                ...
                {recentActivity.map(activity => (
                  <div key={activity.id} className="flex items-center gap-3 text-sm">
                    ...
                    <span className="flex-1">
                      <span className="font-medium">{activity.resume.name || 'Untitled'}</span>
                      {' '}was {activity.type}
                    </span>
          ...
          <div className="flex items-center gap-3 p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
            ...
            <p className="text-sm text-red-700 dark:text-red-300">
              {selectedResumes.size > 1 
                ? `This will permanently delete ${selectedResumes.size} resumes.`
                : 'This action cannot be undone. This resume will be permanently deleted.'}
            </p>
          </div>
